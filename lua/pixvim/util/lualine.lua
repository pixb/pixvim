...
function M.cmp_source(name, icon)
  icon = icon or PixVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    path = PixVim.norm(path)
    local root = PixVim.root.get({ normalize = true })
    local cwd = PixVim.root.cwd()

    -- original path is preserved to provide user with expected result of pretty_path, not a normalized one,
    -- which might be confusing
    local norm_path = path

    if PixVim.is_win() then
