...
function M.info(buf)
  ...
  PixVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    PixVim.warn("No formatter available", { title = "PixVim" })
  end
end
...
function M.health()
  ...
  if has_plugin and not has_extra then
    PixVim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in PixVim.",
      "",
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return PixVim.format.enabled()
    end,
    set = function(state)
      PixVim.format.enable(state, buf)
    end,
  })
end
