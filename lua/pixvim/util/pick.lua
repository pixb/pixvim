...
function M.open(command, opts)
  if not M.picker then
    return PixVim.error("LazyVim.pick: picker not set")
  end
  ...
  if type(opts.cwd) == "boolean" then
    PixVim.warn("LazyVim.pick: opts.cwd should be a string or nil")
    opts.cwd = nil
  end

  if not opts.cwd and opts.root ~= false then
    opts.cwd = PixVim.root({ buf = opts.buf })
  end
...
function M.wrap(command, opts)
  opts = opts or {}
  return function()
    PixVim.pick.open(command, vim.deepcopy(opts))
  end
end
