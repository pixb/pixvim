...
function M.fix_imports()
  ...
  Plugin.Spec.import = PixVim.inject.args(Plugin.Spec.import, function(_, spec)
    if M.handle_defaults and PixVim.config.json.loaded then
      -- extra disabled by defaults?
      defaults = defaults or PixVim.config.get_defaults()
    ...
    if dep then
      ...
      PixVim.warn(dep, { title = "PixVim", once = true, stacktrace = true, stacklevel = 6 })
      return false
    end
  end)
end
...
function M.fix_renames()
  ...
  Meta.add = PixVim.inject.args(Meta.add, function(self, plugin)
